...
// Средний расход топлива для контейнеровоза в тоннах на 1000 морских миль
// Значения скорректированы на основе данных о современных эффективных судах
const FUEL_CONSUMPTION_RATES = {
  'small': 12,  // Малые суда (до 1000 TEU)
  'medium': 20, // Средние суда (1000-3000 TEU)
  'large': 40,  // Большие суда (3000-8000 TEU)
  'vlarge': 60  // Очень большие суда (более 8000 TEU)
};

// Коэффициенты для разных типов маршрутов
const ROUTE_FACTORS = {
  'transpacific': 1.2,    // Транстихоокеанские маршруты
  'transatlantic': 1.1,   // Трансатлантические маршруты
  'asia_europe': 1.3,     // Маршруты Азия-Европа
  'intra_asia': 0.9,      // Внутриазиатские маршруты
  'intra_europe': 0.8,    // Внутриевропейские маршруты
  'other': 1.0            // Другие маршруты
};
...
async function initializeFuelSurchargeTables() {
  ...
  try {
    ...
    // Создание таблицы топливных надбавок, если она не существует
    await client.query(`
      CREATE TABLE IF NOT EXISTS fuel_surcharges (
        ...
        expiry_date DATE,
        created_at TIMESTAMP NOT NULL DEFAULT NOW(),
        UNIQUE(origin_region, destination_region, container_type, fuel_type, effective_date)
      )
    `);
...
// Функция для определения типа маршрута на основе регионов портов
async function determineRouteType(originPortId, destinationPortId) {
  try {
    // Получение регионов портов
    const query = `
      SELECT p1.region as origin_region, p2.region as destination_region
    ...
    if (result.rows.length > 0) {
      const { origin_region, destination_region } = result.rows[0];
      
      // Определение типа маршрута на основе регионов
      if ((origin_region === 'Asia' && destination_region === 'North America') || 
          (origin_region === 'North America' && destination_region === 'Asia')) {
        return 'transpacific';
      } else if ((origin_region === 'Europe' && destination_region === 'North America') || 
                (origin_region === 'North America' && destination_region === 'Europe')) {
        return 'transatlantic';
      } else if ((origin_region === 'Asia' && destination_region === 'Europe') || 
                (origin_region === 'Europe' && destination_region === 'Asia')) {
        return 'asia_europe';
      } else if (origin_region === 'Asia' && destination_region === 'Asia') {
        return 'intra_asia';
      } else if (origin_region === 'Europe' && destination_region === 'Europe') {
        return 'intra_europe';
      }
    }
    
    // По умолчанию возвращаем 'other'
    return 'other';
  } catch (error) {
    console.error('Error determining route type:', error);
    return 'other';
  }
}

// Функция для определения размера судна на основе расстояния и объема перевозок
function determineVesselSize(distance) {
  // Определение размера судна на основе расстояния
  // Обычно на длинных маршрутах используются более крупные суда
  if (distance > 10000) {
    return 'vlarge'; // Очень большие суда для межконтинентальных маршрутов
  } else if (distance > 5000) {
    return 'large';  // Большие суда для дальних маршрутов
  } else if (distance > 2000) {
    return 'medium'; // Средние суда для региональных маршрутов
  } else {
    return 'small';  // Малые суда для коротких маршрутов
  }
}

// Функция для расчета топливной надбавки (скорректированная версия)
async function calculateFuelSurcharge(originPortId, destinationPortId, containerType, fuelType = 'VLSFO') {
  try {
    ...
    const routeType = await determineRouteType(originPortId, destinationPortId);
    
    // Определение размера судна на основе расстояния
    const vesselSize = determineVesselSize(distance);
    
    // Получение коэффициента расхода топлива для данного размера судна
    const fuelConsumptionRate = FUEL_CONSUMPTION_RATES[vesselSize];
    
    // Получение коэффициента для данного типа маршрута
    const routeFactor = ROUTE_FACTORS[routeType] || ROUTE_FACTORS['other'];
    ...
    // Расчет топливной надбавки по скорректированной формуле
    // Формула: (текущая цена - базовая цена) * расход топлива * коэффициент маршрута * коэффициент контейнера * (расстояние / 1000) * 0.001
    const fuelDifference = Math.max(0, currentFuelPrice - BASE_FUEL_PRICE);
    const surcharge = fuelDifference * fuelConsumptionRate * routeFactor * containerFactor * (distance / 1000) * 0.001;
    ...
    // Сохранение рассчитанной надбавки в базу данных
    await saveFuelSurcharge(originPortId, destinationPortId, containerType, roundedSurcharge, currentFuelPrice, fuelType);
...
// Функция для сохранения рассчитанной топливной надбавки в базу данных
async function saveFuelSurcharge(originPortId, destinationPortId, containerType, surcharge, fuelPrice, fuelType) {
  try {
    // Получение регионов портов
    const query = `
      SELECT p1.region as origin_region, p2.region as destination_region
    ...
    if (result.rows.length > 0) {
      ...
      // Вставка данных в таблицу топливных надбавок
      await pool.query(
        `INSERT INTO fuel_surcharges 
         (origin_region, destination_region, container_type, surcharge, fuel_price, fuel_type, effective_date) 
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (origin_region, destination_region, container_type, fuel_type, effective_date) 
         DO UPDATE SET 
           surcharge = $4,
           fuel_price = $5`,
        [
